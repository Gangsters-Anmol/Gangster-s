...
async function _submitPost() {
  ...
  if (!text && _pendingFiles.length === 0) return showToast("Write something or attach media!");
  ...
  try {
    ...
    if (pollPayload) {
      const pollBtn = document.getElementById("pwPostBtn");
      if (pollBtn) { pollBtn.innerHTML = "📊 Poll"; pollBtn.classList.remove("has-poll"); }
    }
...
async function _loadMorePosts() {
  ...
  try {
    ...
    snap.forEach(d => {
      const card = _buildCard(d.id, d.data());
      if (card) postsFeed.insertBefore(card, sentinel);
    });
  } catch { loader.remove(); }
}
...
function _buildCard(id, data) {
  ...
  card.innerHTML = `
    ...
    ${mediaHTML}
    ${data.poll ? `<div class="pw-poll-inject" data-post-id="${id}"></div>` : ""}
  ...
  // Hydrate poll widget if this post has a poll
  if (data.poll && window.renderPollWidget) {
    const ph = card.querySelector(".pw-poll-inject");
    if (ph) {
      ph.innerHTML = window.renderPollWidget(data.poll, id, "posts");
    }
  }
