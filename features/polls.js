...
import {
  ...
  serverTimestamp, getDoc, getDocs, deleteField, query, where
...
/* ─── Base poll card (used in chat bubbles) ─────────────── */
.wap-poll {
  ...
  box-shadow: 0 3px 18px rgba(100,80,140,.12), 0 1px 0 rgba(255,255,255,.75) inset;
  -webkit-user-select: none;
  user-select: none;
  transition: box-shadow .25s;
}
.wap-poll:hover {
  box-shadow: 0 5px 28px rgba(100,80,140,.18), 0 1px 0 rgba(255,255,255,.75) inset;
}
.mine .wap-poll {
  background: rgba(255,255,255,0.86);
  border-color: rgba(167,139,202,0.32);
}

/* Dark mode — chat */
[data-theme="dark"] .wap-poll {
  background: rgba(28,24,48,0.82);
  border-color: rgba(167,139,202,0.2);
  box-shadow: 0 3px 18px rgba(0,0,0,.3), 0 1px 0 rgba(255,255,255,.05) inset;
}
[data-theme="dark"] .mine .wap-poll {
  background: rgba(38,30,62,0.88);
}

/* ─── Chat poll bubble: wider + bigger ───────────────────── */
.chat-bubble-wrap:has(.wap-poll-ph),
.chat-bubble-wrap:has(.wap-poll) {
  max-width: 92% !important;
  width: 92%;
}
.chat-bubble-wrap:has(.wap-poll-ph) .chat-bubble,
.chat-bubble-wrap:has(.wap-poll) .chat-bubble {
...
/* ─── Post feed poll: seamless, full-width ───────────────── */
.wap-post-poll {
  margin: .6rem 0 .2rem;
  width: 100%;
}
.wap-post-poll .wap-poll {
  /* Seamlessly blends into the post card — no nested card effect */
...
.wap-post-poll .wap-poll:hover {
  background: rgba(167,139,202,0.10);
  box-shadow: none;
}
[data-theme="dark"] .wap-post-poll .wap-poll {
  background: rgba(167,139,202,0.07);
  border-color: rgba(167,139,202,0.16);
  box-shadow: none;
}
[data-theme="dark"] .wap-post-poll .wap-poll:hover {
  background: rgba(167,139,202,0.12);
}

/* Bigger text in posts */
.wap-post-poll .wap-poll-q { font-size: 1.02rem; }
.wap-post-poll .wap-poll-opt { padding: .6rem 1.05rem .5rem; }
.wap-post-poll .wap-poll-txt { font-size: .92rem; }
.wap-post-poll .wap-poll-track { height: 6px; border-radius: 3px; }
.wap-post-poll .wap-view-btn { font-size: .88rem; padding: .78rem 1rem; }

.wap-poll-top { padding: .92rem 1.1rem .62rem; }
.wap-poll-q {
  font-size: 1rem; font-weight: 700;
  color: var(--clr-text,#2e2c3a);
  line-height: 1.38; margin-bottom: .36rem;
}
[data-theme="dark"] .wap-poll-q { color: #f0eef8; }

.wap-poll-sub {
  display: flex; align-items: center; gap: .3rem;
  font-size: .69rem; color: var(--clr-muted,#7a7590);
  font-weight: 500;
}
[data-theme="dark"] .wap-poll-sub { color: #9d92b8; }

/* ─── Options ───────────────────────────────────────────── */
.wap-poll-opts { padding: .05rem 0 .15rem; }

.wap-poll-opt {
  display: flex; flex-direction: column;
  padding: .56rem 1.1rem .46rem;
  cursor: pointer; border: none;
  background: transparent; width: 100%; text-align: left;
  transition: background .16s;
  position: relative;
}
.wap-poll-opt:hover:not([disabled]) {
  background: rgba(167,139,202,.07);
}
[data-theme="dark"] .wap-poll-opt:hover:not([disabled]) {
  background: rgba(167,139,202,.10);
}
.wap-poll-opt[disabled] { cursor: default; }

.wap-poll-opt-row {
  display: flex; align-items: center;
  gap: .65rem; margin-bottom: .3rem; position: relative; z-index: 1;
}
...
.wap-poll-opt:hover:not([disabled]) .wap-poll-radio {
  border-color: var(--clr-accent2,#f0a8b0);
  transform: scale(1.08);
}
.wap-poll-opt.sel .wap-poll-radio {
...
.wap-poll-opt.sel .wap-poll-dot { opacity: 1; transform: scale(1); }

.wap-poll-txt {
  flex: 1; font-size: .9rem;
  color: var(--clr-text,#2e2c3a);
  line-height: 1.3; font-weight: 450;
}
[data-theme="dark"] .wap-poll-txt { color: #e8e4f4; }

.wap-poll-cnt {
  font-size: .78rem; color: var(--clr-muted,#7a7590);
...
.wap-poll-track {
  height: 6px;
  background: rgba(167,139,202,.14);
  border-radius: 3px; overflow: hidden;
  position: relative; z-index: 0;
}
[data-theme="dark"] .wap-poll-track { background: rgba(255,255,255,.1); }

.wap-poll-fill {
  height: 100%; border-radius: 3px;
  background: linear-gradient(90deg, rgba(167,139,202,.45), rgba(240,168,176,.45));
  transition: width .55s cubic-bezier(.16,1,.3,1);
}
.wap-poll-opt.sel .wap-poll-fill {
  background: linear-gradient(90deg, var(--clr-accent,#a78bca), var(--clr-accent2,#f0a8b0));
  opacity: .7;
}
...
.wap-poll-meta {
  ...
  padding: .3rem .95rem .55rem;
  font-size: .64rem;
  color: var(--clr-muted,#7a7590);
  gap: .4rem;
}
[data-theme="dark"] .wap-poll-meta { color: #9d92b8; }
...
.wap-poll-ended {
  ...
  border-radius: 50px; padding: .1rem .48rem;
}
.wap-change-vote {
  background: none; border: none; cursor: pointer;
  color: var(--clr-muted,#7a7590); font-size: .64rem;
  font-family: var(--font-body,'Jost',sans-serif);
  padding: .1rem .38rem; border-radius: 5px;
  margin-right: auto; transition: color .15s, background .15s;
}
.wap-change-vote:hover {
...
.wap-poll-divider {
  height: 1px;
  background: rgba(167,139,202,.18);
  margin: 0 .65rem;
}
[data-theme="dark"] .wap-poll-divider { background: rgba(255,255,255,.08); }
...
[data-theme="dark"] .wap-view-btn { color: #c4aee8; }
[data-theme="dark"] .wap-view-btn:hover {
  background: rgba(167,139,202,.12);
  color: #f0eef8;
}
...
/* ══════════════════════════════════════════════════════════════
   4b. window.hydratePollCard — called by posts.js directly
   This is the GUARANTEED way to render a poll in a post card.
   posts.js calls this after the card is in the DOM.
══════════════════════════════════════════════════════════════ */
window.hydratePollCard = function(hostEl) {
  const docId   = hostEl?.dataset?.doc;
  const colName = hostEl?.dataset?.col;
  if (!docId || !colName) return;
  // Remove data attrs so observer doesn't double-process
  delete hostEl.dataset.doc;
  delete hostEl.dataset.col;
  _attachLive(hostEl, docId, colName);
};

/* ══════════════════════════════════════════════════════════════
   5.  SINGLE MutationObserver on document.body
   Catches every .wap-poll-ph and .wap-post-poll anywhere, always.
   No timing race: fires for every DOM insertion on the whole page.
══════════════════════════════════════════════════════════════ */
...
// Start observer immediately — watch entire document body
const _bodyObs = new MutationObserver(() => _upgradeRoot(document.body));
_bodyObs.observe(document.body, { childList: true, subtree: true });
// Also sweep anything already in DOM right now
_upgradeRoot(document.body);
...
function _pollHTML(poll, docId, colName, time) {
  ...
  const subIcon = poll.allowMultiple
    ? `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--clr-muted,#7a7590)" stroke-width="2.2"><polyline points="20 6 9 17 4 12"/><polyline points="20 12 9 23 4 18"/></svg> Select one or more`
    : `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--clr-muted,#7a7590)" stroke-width="2.2"><polyline points="20 6 9 17 4 12"/></svg> Select one`;
...
onAuthChange((user, profile) => {
  ...
  // Watch for the create-post-panel to gain postMediaRow (posts.js injects it on auth)
  const tryInjectPost = () => {
    if (document.getElementById('wapPollPostBtn')) return;
    // Prefer postMediaRow (exists for specialUser); fallback to create-post-footer
    const row = document.getElementById('postMediaRow')
             || document.querySelector('.create-post-footer');
    if (!row) { setTimeout(tryInjectPost, 250); return; }
    ...
    // Insert before the submit button (or first child if no submit)
    const submitBtn = row.querySelector('#submitPostBtn') || row.firstChild;
    row.insertBefore(btn, submitBtn);
  };

  // Also watch via MutationObserver so we catch postMediaRow the instant it's injected
  const postPanel = document.querySelector('.create-post-panel');
  if (postPanel) {
    const obs = new MutationObserver(() => {
      if (!document.getElementById('wapPollPostBtn') && document.getElementById('postMediaRow')) {
        tryInjectPost();
      }
    });
    obs.observe(postPanel, { childList: true, subtree: true });
  }
  tryInjectPost();
    ...
    btn.addEventListener('click', () => {
      ...
      _openCreator('Post Poll to Chat', async data => {
        try {
          ...
          // Notify all currently online users in the group chat
          try {
            const onlineSnap = await getDocs(query(collection(db, 'users'), where('online', '==', true)));
            onlineSnap.forEach(d => {
              if (d.id !== currentUser.uid) {
                sendNotification(d.id, {
                  ...
                  text:     data.question?.slice(0, 60) || 'A new poll',
                });
              }
            });
