...
import { sendNotification }              from "./features/notifications.js";
...
async function _sendPrivate() {
  ...
  // Notify the recipient about the new private message
  sendNotification(_activePrivate.uid, {
...
function _appendBubble(container, msgId, data, type, convId) {
  ...
  if (data.type === "poll" && data.poll) {
    ...
    // Build bubble structure with DOM methods (not innerHTML) so the
    // MutationObserver in polls.js can see the placeholder and upgrade it
    if (!mine) {
      const author = document.createElement("span");
      author.className = "chat-bubble__author";
      author.innerHTML = `${_esc(data.name)} <em class="chat-bubble__role-tag">${_esc(data.role)}</em>`;
      bubble.appendChild(author);
    }
    // Poll placeholder — polls.js MutationObserver upgrades this to a live widget
    const ph = document.createElement("div");
    ph.className = "wap-poll-ph";
    ph.dataset.doc = msgId;
    ph.dataset.col = collection_name;
    // Pre-render static snapshot so it's not blank while observer fires
    if (window.renderPollWidget) {
      ph.innerHTML = window.renderPollWidget(data.poll, msgId, collection_name)
